...
class FaithfulnessJudge(BaseJudge):
    ...
    USER_TEMPLATE = (
        ...
        "   - Each atomic claim should contain only one checkable fact.\n"
        "   - Split combined statements whenever they contain more than one factual assertion.\n"
        "   - Do NOT split names or fixed expressions unnaturally.\n"
        ...
        "3. For each claim, check if it is supported by the CONTEXTS using STRICT rules:\n"
        "   - Carefully read the entire set of CONTEXTS.\n"
        "   - A claim is supported ONLY if the full factual meaning matches the CONTEXTS.\n"
        "   - All material elements of the claim must be supported together, including when present:\n"
        "       * subject / actor\n"
        "       * event / action\n"
        "       * object / target\n"
        "       * date / year / time reference\n"
        "       * location\n"
        "       * causal relation\n"
        "       * quantity / scope\n"
        "       * modality or certainty (e.g. exact, definitive, immediate)\n"
        "   - Entities (people, places, organizations) must match in meaning.\n"
        "   - Temporal information (years, dates, sequence markers like 'before', 'after', 'immediately') is MATERIAL and must match exactly.\n"
        "   - Causal claims must be explicitly and strictly supported as causal, not merely related by topic.\n"
        "   - Verbatim or near-verbatim overlap is NOT enough if any material detail is changed, overstated, omitted, or contradicted.\n"
        "   - If only part of a claim is supported, the claim is NOT supported.\n"
        "   - Support must come ONLY from the CONTEXTS.\n"
        "   - If the context does not explicitly support the full claim, or contradicts any material part of it, mark it as hallucinated.\n\n"

        "4. Classify each claim into:\n"
        "   - \"supported\": fully supported by the context.\n"
        "   - \"partial_hallucination\": some parts supported but at least one material factual element unsupported, altered, overstated, or contradicted.\n"
        "   - \"full_hallucination\": not supported at all or clearly contradicted by the context.\n\n"

        "5. Compute strict_faithfulness = (# supported claims) / (total number of claims).\n"
        "   - ONLY claims labeled \"supported\" count toward the score.\n"
        "   - Claims labeled partial_hallucination or full_hallucination do NOT count as supported.\n\n"

        "6. Before returning, verify internally:\n"
        "   - Every claim came directly from the ANSWER.\n"
        "   - Every claim was checked against the FULL CONTEXTS.\n"
        "   - No claim is marked supported if any material detail mismatches.\n"
        "   - The numeric score exactly matches the number of supported claims divided by total claims.\n\n"

        "7. Return JSON ONLY in this exact format:\n"
        "{{\n"
